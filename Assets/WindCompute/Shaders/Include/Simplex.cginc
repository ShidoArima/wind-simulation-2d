...
float damper_exponential(
    ...
    float ft = 1.0f / 60.0f)
{
    return lerp(x, g, 1.0f - pow(1.0 / (1.0 - ft * damping), -dt / ft));
}
...
float2 damper_exact(float2 current, float2 target, float halflife, float dt)
{
    float2 value;
    value.x = damper_exact(current.x, target.x, halflife, dt);
    value.y = damper_exact(current.x, target.x, halflife, dt);
